...
class TIOInversion:
    ...
    def export_pair_to_binary(self, pair, overwrite: bool = False) -> tuple[Path, Path]:
        ...
        if not overwrite and ew_bin.exists() and ns_bin.exists():
            logger.info(f"Pair {pair.pa_key}: binary files already exist, skipping (use overwrite=True to redo)")
    ...
    def write_liste_couple(
        ...
        of image dates.  The file is written to both ``inverse_EW/`` and
        ``inverse_NS/`` and marked executable.

        :param weight_mode: Weighting strategy.  One of:
        ...
        :param invert: When ``True``, reverses the weighting direction for
            ``'relative_temporal'`` and ``'sigmoid'`` by substituting
            ``t_norm → 1 − t_norm`` before applying the formula.  Longer
            pairs get higher weight, shorter pairs lower — useful for slow
            phenomena (rock glaciers, slow landslides) where a larger temporal
            baseline improves displacement SNR.  No effect on ``'uniform'``,
    ...
    def prepare(
        ...
        :param weight_mode: Pair weighting strategy passed to
            :meth:`write_liste_couple`.  One of ``'uniform'``,
            ``'temporal'``, ``'relative_temporal'``, ``'sigmoid'``, or
            ``'parametric'``.
        :type weight_mode: str
        ...
        :param invert: Reverse weighting direction for ``'relative_temporal'``
            and ``'sigmoid'`` so longer Δt pairs receive higher weight.
        :type invert: bool
