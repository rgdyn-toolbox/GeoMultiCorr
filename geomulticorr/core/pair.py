...
class Pair:
    ...
    def __init__(self, session=None, target_path=None, left=None, right=None):
        ...
        if type(target_path) in (str, Path):
            ...
            assert Path(target_path).parent.exists() and Path(target_path).parent.name.lower() == 'image_correlation', 'this path is not leading to a well-formed pair'
        ...
        self.pa_direction = "forward" if left.th_date_datetime <= right.th_date_datetime else "backward"
...
"""
    ...
    def get_disp_corr_geoim(self):
            self.pa_disparity_f_geoim = rt.Open(str(self.pa_disparity_f_path), load_pixels=True, nBands=3)
            return self.pa_disparity_f_geoim

    def get_dispX_geoim(self):
        try:
            return self.pa_dispX_geoim
        except AttributeError:
            self.pa_dispX_geoim = rt.Open(str(self.pa_disparity_f_path), load_pixels=True, nBands=1)
            return self.pa_dispX_geoim
    ...
    def get_dispY_geoim(self):
        try:
            ...
            self.pa_dispY_geoim = rt.Open(str(self.pa_disparity_f_path), load_pixels=True, nBands=2)
            return self.pa_dispY_geoim
    
    def get_vx_geoim(self):
        return self.get_dispX_geoim() / self.pa_dt_years

    def get_vy_geoim(self):
        return self.get_dispY_geoim() / self.pa_dt_years

    def get_vmagn_geoim(self):
        return self.get_magn_geoim() / self.pa_dt_years
    ...
    def compute_magnitude(self):
        ...
        xDisp, yDisp = rt.Open(str(self.pa_disparity_f_path), nBands=[1,2], load_pixels=True).splitBands()
